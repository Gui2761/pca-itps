...
class ResourceTypeTab extends StatefulWidget {
  const ResourceTypeTab({super.key});

  @override
  State<ResourceTypeTab> createState() => _ResourceTypeTabState();
}

class _ResourceTypeTabState extends State<ResourceTypeTab> {
  ...
  Future<void> _addItem() async {
    ...
    final success = await _apiService.createTipoRecurso(nome);
    ...
    if (success) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Tipo de recurso inserido com sucesso!', style: GoogleFonts.inter(fontWeight: FontWeight.w600)),
  ...
  Future<void> _deleteItem(int id, String nome) async {
    ...
    if (confirm == true) {
      setState(() => _isLoading = true);
      final success = await _apiService.deleteTipoRecurso(id);
      ...
      if (success) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Erro ao excluir tipo de recurso.', style: GoogleFonts.inter(fontWeight: FontWeight.w600)),
              backgroundColor: const Color(0xFFDC2626),
              behavior: SnackBarBehavior.floating,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
            ),
