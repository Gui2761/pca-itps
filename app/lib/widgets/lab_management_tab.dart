...
class _LabManagementTabState extends State<LabManagementTab> {
  ...
  Future<void> _addItem() async {
    ...
    final success = await _apiService.createLaboratorio(nome);
    ...
    if (success) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Laboratório inserido com sucesso!', style: GoogleFonts.inter(fontWeight: FontWeight.w600)),
  ...
  Future<void> _deleteItem(int id, String nome) async {
    ...
    if (confirm == true) {
      setState(() => _isLoading = true);
      final success = await _apiService.deleteLaboratorio(id);
      ...
      if (success) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Erro ao excluir laboratório.', style: GoogleFonts.inter(fontWeight: FontWeight.w600)),
              backgroundColor: const Color(0xFFDC2626),
              behavior: SnackBarBehavior.floating,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
            ),
  ...
  Widget _buildParameterList() {
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        final item = _laboratorios[index];
